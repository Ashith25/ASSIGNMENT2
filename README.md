 ...
 * Check for missing values in the 'Price' column. How would you handle products with missing price information?
    ...
    Finding out the missing price cells by using the Syntax **=ISBLANK(value)**

   Missing price is replaced with Median **=IF(ISBLANK(Table1[@PRICE]),MEDIAN(Table1[PRICE]),Table1[@PRICE])**

* If there are products with missing categories, propose a strategy to impute or deal with these missing values effectively.
  
  Missing values shows as “Unknown” Syntax **=IF(ISBLANK(F2), "Unknown",F2)**

## Correcting Inconsistent Data:
* Identify any inconsistent text formats present in the "Product Name" column.
  
  Arranged the “Product name” column with PROPER function **Syntax =PROPER(B2)**
...
Removed Duplicate Rows **Data – Remove Duplicates**
...
**Syntax =DATE(2026,MONTH(MID(A2,4,3)&"1"),LEFT(A2,2))**
